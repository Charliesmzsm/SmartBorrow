...
/* harmony import */ var _index_vue_vue_type_style_index_0_id_3523e786_scoped_true_lang_css___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./index.vue?vue&type=style&index=0&id=3523e786&scoped=true&lang=css& */ 43);
/* harmony import */ var _F_HBuilderX_plugins_uniapp_cli_node_modules_dcloudio_vue_cli_plugin_uni_packages_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./node_modules/@dcloudio/vue-cli-plugin-uni/packages/vue-loader/lib/runtime/componentNormalizer.js */ 14);
...
var components = {
  tarbar: function() {
    return Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! @/components/tarbar/tarbar.vue */ 15))
  }
}
var render = function() {
  var _vm = this
...
var _helper = _interopRequireDefault(__webpack_require__(/*! ../../common/helper.js */ 42));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} //
//
//
//
//
...
//
//
var _default = { data: function data() {return { title: 'Hello', imageUrl: [], alreadyUseInfo: { image: '../../static/camara.jpg', place: '数媒实验室618', number: '111', time: Date().split(0, 1), belong: '浙江科技学院', type: true }, freeItem: [], state: [{ name: '借出中', type: 2 }, { name: '空闲中', type: 1 }, { name: '维护中', type: 3 }], ifBorrow: false, ifUse: false // 用户是否登录
    };}, onLoad: function onLoad() {}, methods: { search: function search() {uni.navigateTo({ url: "../search/search" });}, back: function back() {1;uni.showToast({ title: '这是归还' });uni.clearStorage({ key: 'user', success: function success() {uni.showToast({ title: "清空本地记录成功", icon: 'none' });} });}, delayed: function delayed() {uni.showToast({ title: '这是延时' });}, // getInfoLogin(){
    // 	let user=uni.getStorageSync('user')
    // 	if()
    // 	return uni.getStorageSync('ifLogin')
    // },
    getDeviceInfo: function getDeviceInfo(id) {var device_id = JSON.stringify(id);uni.navigateTo({ url: '../info/info?id=' + device_id });}, getOpenId: function getOpenId() {return uni.getStorageSync('openid');}, getUserInfo: function getUserInfo() {// 通过oppenid 获取 用户信息，如果openid不存在，则跳转至
      var openid = this.getOpenId();var that = this;uni.request({ url: that.configUrl() + 'miniapp/user/info/' + openid, success: function success(res) {uni.setStorage({ key: 'user', data: res.data.data, success: function success() {}, fail: function fail() {uni.showTotast({ title: "获取用户信息失败", icon: 'none' });} });} });}, updateUserInfo: function updateUserInfo() {// 修改个人信息
      if (this.getOpenId()) {var that = this;var appid, secret;uni.request({ url: that.configUrl() + 'miniapp/common/open', method: 'GET', success: function success(res) {if (res.data.data.length === 1) {var result = res.data.data;appid = result[0].appId;
        ...
        wx.getUserInfo({
          success: function success(res) {
            ...
            uni.request({
              url: that.configUrl() + 'miniapp/user/change/info?open_id=' + that.getOpenId() + '&wx_name=' + result.nickName + '&wx_pic=' + result.avatarUrl + '&sex=' + result.gender,
              method: 'POST' });

          } });

        // uni.request({
        // 	url:that.configUrl()+'miniapp/user/change/info',
        // 	data:{
        // 		open_id:that.getOpenId()
        ...
        success: function success(res) {
          if (res.data.data.length) {
            ...
            res.data.data.forEach(function (item) {
              ...
              _helper.default.setUserDeviceTiem(d);
              var f = _helper.default.getTime(d);
              var e = new Date();
              var timeDif = _helper.default.GetDateTimeDiff(d, e);
              timeDif.Hours = 24 - timeDif.Hours;
              timeDif.Minutes = 60 - timeDif.Minutes;
              timeDif.Seconds = 60 - timeDif.Seconds;

              that.alreadyUseInfo = {
                place: item.position,
                number: item.device_number,
                timeH: timeDif.Hours < 10 ? '0' + timeDif.Hours : timeDif.Hours,
                timeM: timeDif.Minutes < 10 ? '0' + timeDif.Minutes : timeDif.Minutes,
                timeS: timeDif.Seconds < 10 ? '0' + timeDif.Seconds : timeDif.Seconds,
                type: timeDif.Days >= 1 ? false : true,
                belong: item.belong,
                status: item.status,
    ...
    getCarouselimg: function getCarouselimg() {
      ...
      uni.request({
        ...
        success: function success(res) {
          that.imageUrl = [];
          res.data.data.forEach(function (item) {
            that.imageUrl.push(that.configUrlImg() + '/' + item.picture);
          });
        } });
